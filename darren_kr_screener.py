...
================================================================================
GitHub Actions 클라우드 실행용 (야후 파이낸스 + 섹터별 분류 버전).

구조:
- 종목 목록: 저장소에 올려둔 kr_tickers.csv 에서 읽음
             (make_kr_tickers.py로 본인 PC/코랩에서 1회 생성)
- 가격 데이터: 야후 파이낸스(005930.KS / 035720.KQ 형식)로 조회
- 섹터 정보: 야후 파이낸스에서 조회 (한국 종목은 일부 미분류 가능)

변경점(v3):
- 최종 통과 종목에 섹터 정보를 붙이고, 섹터별로 묶어 섹터 내 거래대금순 정렬.
...
def run_kr_scan(verbose: bool = True) -> tuple[pd.DataFrame, str]:
    ...
    meta = {row["code"]: (row["name"], row["market"]) for _, row in tk.iterrows()}
